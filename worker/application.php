...
<?php
  ...
  if(isset($_SESSION['hw_id'])) {
    ...
    $result = $mysqli->query("SELECT P.PATIENT_ID, P.PAT_STATUS, P.PAT_FNAME, P.PAT_LNAME, V.APP_ID, V.APP_DATE, V.APP_VACCICODE 
    FROM VACCI_APP V,PATIENT P WHERE V.PATIENT_ID = P.PATIENT_ID
    AND V.APP_CITY = '$city' AND P.PAT_STATUS = 'None'") or die($mysqli->error);
   ...
   ?>
    <tr>
      ...
      <?php 
        echo '<input type="hidden" name="patid" value="'.$row['PATIENT_ID'].'">'
      ?>
        <td><button name="decline">Decline</button></td>
      </form>
    </tr>
    <?php endwhile; ?>
