...
<?php
  ...
  if(isset($_SESSION['hw_id'])) {
    ...
    $result = $mysqli->query("SELECT APP_ID, PAT_FNAME, PAT_LNAME, APP_DATE, APP_VACCICODE, PAT_STATUS 
    FROM VACCI_APP V,PATIENT P WHERE V.PATIENT_ID = P.PATIENT_ID
    AND V.APP_CITY = '$city' AND P.PAT_STATUS = 'Approve'") or die($mysqli->error);
